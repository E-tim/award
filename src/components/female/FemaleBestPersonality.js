...
const FemaleBestPersonality = () => {

  const [list, setList] = useState([])

  useEffect(()=> {
    const q = query(collection(db, 'f-bestpersonality'))
  ...
  useEffect(()=>{
    const fetchImages = async () => {
      try {
        const storageRef = ref(storage, 'f-bestpersonality') ;
...
export default FemaleBestPersonality